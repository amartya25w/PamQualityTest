...
namespace ArconConnector.Forms
{
    public partial class FrmLogin : Form
    {
        ...
        private string _LogCode = "FL", _MethodName = string.Empty;
        ...
        public void TerminateApplication(BaseConnector objBaseConnector, bool killCurrentProcess, bool resetSessionTime)
        {
            ...
            try
            {
                ...
                if (cmdArgs[0].ToString().ToLower().IndexOf("arcosloginacmo") > 0 ||
                    cmdArgs[0].ToString().ToLower().IndexOf("arcosscriptmanager") > 0 ||
                    cmdArgs[0].ToString().ToLower().IndexOf("arcosbiometricfingerprintauthenticator") > 0 ||
                    cmdArgs[0].ToString().ToLower().IndexOf("arcosuseraccesslogviewer") > 0)
                {
                }
                else
                {
                    if (objBaseConnector.UserDetails.SessionId > 0)
                    {
                        if (!resetSessionTime)
                        {
                            EndUserServiceSession(objBaseConnector.UserDetails.SessionId);
                        }
        ...
        public void EndUserServiceSession(int sessionId)
        {
            ...
            try
            {
                ...
                objProxyUserServiceSession.EndUserServiceSession(sessionId);
            }
            catch (Exception ex)
            {
                _Log.Error(_LogCode + " " + ex);
